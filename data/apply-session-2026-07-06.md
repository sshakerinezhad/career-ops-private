...
3. **Figure #13: DONE.** Submitted 2026-07-08 (Greenhouse; CV `output/cv-candidate-figure-helix-2026-07-08.pdf` + cover letter `output/figure-helix-ai-engineer-cover.pdf`). Tracker Applied, follow-up pinned 2026-07-15 (batch with OpenAI #4's), answers persisted in report 013. **Haize #22: FULLY PREPPED, awaiting user submit** (CV `output/cv-candidate-haize-2026-07-08.pdf` + cover letter `output/haize-applied-researcher-cover.pdf` both user-approved 2026-07-08; posting re-verified active; form read: resume required, no knock-outs; on submit run the post-submit trio). Then: Cohere #9 (stale 07-05 CV, regenerate first), Skild #14, Gray Swan #21, Mechanize #15, Surge #23 re-check, Reflection #19, Prime Intellect #20.
...
6. **OPEN DECISION (2026-07-08): M.Eng expected-graduation date on CVs — April 2027 vs December 2026.** User says he can genuinely do either; assessment delivered (Dec 2026 recommended IF he commits: tighter relocation story, less student-bucketing). Waiting on his call; if Dec, update main.tex + cv.md + _profile.md rule 8 and regenerate future CVs only (never touch submitted ones).
7. **Voice note:** cover-letter/outreach register + user line-edit lessons captured in `voice-dna.md` §4.5 (2026-07-08) — read it before drafting any user-facing prose.
8. **Infra note (remote env):** /opt/pw-browsers ships browser rev 1194 but repo Playwright wants 1228 — fixed via symlink `/opt/pw-browsers/chromium_headless_shell-1228/chrome-headless-shell-linux64/chrome-headless-shell → .../chromium_headless_shell-1194/chrome-linux/headless_shell` (+ INSTALLATION_COMPLETE/DEPENDENCIES_VALIDATED markers). Ephemeral container: re-create if PDF generation hits "Executable doesn't exist"; never run `npx playwright install`.
...
| 22 | Haize — Applied Researcher | 4.5 | Greenhouse | ✅ `output/cv-candidate-haize-2026-07-08.pdf` | Liveness re-verified + form read 2026-07-08: no knock-outs, no work-auth, resume required, cover letter optional |
