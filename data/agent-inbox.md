...
**Shayan's 09-18 answers, folded into the plan:** Salman = robotics research engineer at DeepMind, gives time Sat 26 and Sun 27 (P9 audience, T10 post-training mock Sat 14:00 PT, A9 algorithms mock Sun morning); Wed 23 evening busy, rest of the day free (rehearsal 2 + GPU run + two algorithm blocks); GPU: he will rent one (T13 scheduled Wed 23 13:00 to 16:00, conditional on rehearsal 2 at 12+ and drills averaging 12+); travel laptop NOT secured yet (device tag renamed to "dev" = any machine with keys; L8 sets Thu 24 as the deadline); algorithms "will need serious work, go broad" (A10 breadth sweep added, A7 target 12 rounds, split now about 38/30/32; §8.2 prompt rotates two extra reported families). He questioned the public repo (reasoning in plan §8 item 1, decision open) and did not name a Toronto mock partner.

**Not yet written, awaiting Shayan's go (plan §8 item 5):** `mercor-onsite/run-sheet.md` (Sat 19 09:30, with him), `mercor-onsite/tradeoff-drills.md` (Sat 19), algorithm drills 7 to 10 + keys (Sun 20), `mercor-onsite/grpo-run.md` (Tue 22), `mock-2-brief.md` + key (Tue 22), `mock-3-brief.md` + key + planted-bug judge (Fri 25). Inputs for all five are in the session-12 handoff below (items b to g). **Open questions to him:** plan §8 items 1 to 7 (mock friends, course markers, Wed 23, keys tested from the travel laptop, GPU for T13, rehearsal 1 moved to Sun 20, toolkit public). career-ops update 1.24.0 → 1.33.0 offered again 09-18, unanswered.
