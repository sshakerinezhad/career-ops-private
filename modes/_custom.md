...
- **Mercor onsite prep: tracking lives in this repo (2026-09-18).** Trigger: `/mercor-prep` (project skill in `.claude/skills/mercor-prep/SKILL.md`), or any message about the prep plan. Plan: `interview-prep/mercor-onsite-plan.md` (rules, schedule §6, day of). Cards, one per task ID, with Read / Do / Drive home / Master / Bot / Test / Done when: `interview-prep/mercor-onsite/practical-track.md`, `algorithms-track.md`, `post-training-track.md`. Ledger (the running thread): `interview-prep/mercor-onsite/ledger.md`. When Shayan reports progress, a score, or asks what is next, in any session:
...
- **Parallel evaluation workers: isolated scratch files + JD integrity check (added 2026-09-02).** Two collisions were caught today: parallel batch workers writing the same scratchpad filename (`jd.txt`, `job.json`) overwrote each other's fetched JDs mid-run. Every worker prompt must (1) name a scratch file prefixed with its own report number and never a generic name, (2) fetch the JD via the ATS API and assert the job id / expected title programmatically before reading content, (3) for Greenhouse also confirm the id against the board-level `/jobs` list, and (4) hard-stop rather than evaluate on any mismatch. Fan-outs stay under 15 workers and use `node reserve-report-num.mjs --count N` first.
