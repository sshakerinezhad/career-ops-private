...
APEX-v1-extended is Mercor's benchmark of expert-written tasks for four jobs: investment banking associate, management consultant, big law associate, primary care physician. The open dev set is `mercor/APEX-v1-extended` on Hugging Face, `data/train.csv`: 100 tasks, 25 per domain, one row each with `Task ID`, `Domain`, `Prompt`, `Rubric JSON`, `File Attachments`. The rubric is a dict, `criterion 1` to `criterion N`, each with `description`, `criterion_type`, `weight`, `dependent_criteria`, `sources`, `justification`, `human_rating`; criteria are binary, Met or Not met. The attachments column lists paths under `documents/<Task ID>/` in the same repo (PDF, CSV, one docx, one xlsx). The leaderboard score is the mean percentage of criteria met, one Judge LM (Gemini 2.5 Pro, thinking on) grading each criterion independently, 8 runs per task.

Use these 30 tasks, the CSV-only tasks with the smallest attachments, so every prompt plus file fits any context and the cost is flat (20 Consulting, 10 Finance, 287 criteria, largest file 82 KB): 145, 283, 352, 772, 828, 1080, 1091, 1149, 1166, 1170, 1171, 1172, 1188, 1192, 1242, 1259, 1265, 1269, 1288, 1384, 2107, 2157, 2192, 2205, 2228, 2230, 2232, 2294, 2302, 2308. Reserve, the other 7 CSV-only tasks: 804, 1122, 1150, 1169, 2121, 2266, 2315.

Two models, your choice, one cheap and one strong, your own keys, one run each.

In 90 minutes, answer:

1. **How do the two models compare on mean percent of criteria met?** Task-level bootstrap CI on each and a paired analysis across the 30 tasks. Say whether you believe the difference is real.
2. **Validate the per-criterion judge.** Label 40 criteria by hand from the models' own outputs. Report accuracy, precision and recall on Met, Cohen's kappa, and the criterion types where it fails.
...
Attachment content is obtainable: every path in the column exists in the repo's `documents/` folder. Mercor's harness (`Mercor-Intelligence/apex-evals`, `apex-evals-v1-extended/`) appends parsed files to the prompt as `==== Attached files content: ====` then one `=== <filename> ===` block per file, and its template says no follow-up questions, attachments are the primary source; read each CSV as text and build the same block. Its judge prompt (`prompt/grading_prompt.txt`) takes criterion description and response only and returns `{"result": 0 or 1, "reason": "..."}`. The card asks that the dataset not be crawled or scraped programmatically; a one-time download for evaluation is its stated intended use.

## Cost and time, before you launch

Calls: generation 30 tasks × 2 models × 1 run = 60; judge 30 × 2 × 9.57 mean criteria = 574. Generation input per call is about (prompt + template chars) / 4 plus attachment bytes / 3.5; the 30 attachments total 779 KB, so about 240k input tokens per model, plus output × 60 (a memo with numbers runs 2,000 to 4,000 tokens). Judge input is about 574 × (response tokens + 50), output about 100 per call. Multiply by the day's prices. Time is calls / concurrency × mean latency: measure latency on a 5-task smoke test, extrapolate, and if the full run needs more than 15 minutes cut tasks, not models.
